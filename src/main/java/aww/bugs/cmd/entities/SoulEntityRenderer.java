...
public class SoulEntityRenderer extends EntityRenderer<SoulEntity> {
	...
	public void render(SoulEntity entity, float yaw, float tickDelta, MatrixStack matrices,
		...
		for (int j = 0; j < Math.round(direction.length() * 4); j++) {
			...
			ClientParticles.spawnWithMaxAge(ParticleTypes.ELECTRIC_SPARK, currentRenderPosition,
					world.random.nextInt((10 + entity.age)));
			if (allParticles)
				ClientParticles.spawn(dust, world, currentRenderPosition, 0.35);
